...
/// only authority level excise can open with today (#324).
/// </summary>
public class PermissionEnforcementTests : IDisposable
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"excise-perm-test-{Guid.NewGuid():N}{suffix}");
        _tempFiles.Add(path);
        return path;
    }
    ...
    {
        // Redaction is deliberately NOT gated on /P (#642): it is excise's
