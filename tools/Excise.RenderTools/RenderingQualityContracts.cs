...
partial class Program
{
    ...
    static Command CreateRenderQualityScanCommand()
    {
        ...
        {
            Description = "Directory for cached excise-rendered PNGs.",
        };
    ...
    internal static bool RunRenderQualityScan(
        ...
        DirectoryInfo? exciseRenderCacheDir,
    ...
    private static bool LooksLikeOutlierPassOne(CorpusScanEntry entry)
    {
        ...
        var rank = entry.exciseReferenceCenterRank.GetValueOrDefault();

        // Hard suspect: excise matches exactly one renderer while the references
        // agree with each other. This is not a reviewed target match.
        if (agreeingOracles == 1 && oraclePairs > 0 && disagreeingPairs == 0)
            return true;

        // Softer suspect: excise's matching renderer is at the edge of a
        // multi-reference comparison. This catches cases such as FreeCulture
        // where excise tracks MuPDF while Poppler/Ghostscript cluster elsewhere.
