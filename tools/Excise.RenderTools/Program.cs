...
partial class Program
{
    ...
    internal static Task<int> RunAsync(string[] args)
    {
        ...
        var rootCommand = new RootCommand("excise renderer/conformance utility commands")
        {
            CreateDrawCommand(),
            CreateDemoCommand(),
    ...
    /// <summary>
    /// Excise.RenderTools draw <file> -o <output.pdf> - Add shapes to PDF (demo)
    /// </summary>
    ...
    /// <summary>
    /// Excise.RenderTools demo - Run interactive demos
    /// </summary>
    static Command CreateDemoCommand()
    {
        var command = new Command("demo", "Run interactive demos of Excise.Core capabilities");

        command.SetAction(_ =>
        {
            Console.WriteLine("=== Excise.Core Demo ===");
            Console.WriteLine();
            ...
            Console.WriteLine("  excise info <file.pdf>              - Show PDF info");
            Console.WriteLine("  excise text <file.pdf>              - Extract text");
            Console.WriteLine("  excise letters <file.pdf> -p 1      - Show letter positions");
            Console.WriteLine("  excise render <file.pdf> -o out.png - Render to image");
            Console.WriteLine("  Excise.RenderTools draw <file.pdf> -o out.pdf - Add shapes");
        });
    ...
    {
        var content = "BT /F1 24 Tf 100 700 Td (Hello from Excise.Core!) Tj ET";
    ...
    /// <summary>
    /// Excise.RenderTools gui-display-hotspots report1.json [report2.json ...]
    ///
    ...
    /// <summary>
    /// Excise.RenderTools corpus-hotspots corpus-scan.json [more.json ...]
    ///
        ...
        foreach (var reportFile in reportFiles)
        {
            ...
            foreach (var entry in entries.EnumerateArray())
            {
                ...
                measuredCurrentMs += AddCorpusCurrentPhase(phaseDurations, cacheCounts, entry,
                    "excise", "renderMs", "exciseCacheHit", "excise-render-cold", "excise-cache-read");
                measuredCurrentMs += AddCorpusCurrentPhase(phaseDurations, cacheCounts, entry,
                ...
                AddCorpusCachedOriginalPhase(phaseDurations, entry, "exciseCachedRenderMs", "excise-render-cached-original");
    ...
    /// <summary>
    /// Excise.RenderTools jbig2-classify &lt;file-or-dir&gt; --output out.json
    ///
    ...
    /// excise binary call (~500 ms startup × 14 = 7 sec).
    ///
    /// Renders selected pages from each PDF in the corpus's chunk-slice
    /// with excise's SkiaRenderer and with reference renderers, computes
    ...
    static Command CreateCorpusScanCommand()
    {
        ...
        {
            Description = "Directory for cached third-party oracle PNGs. Defaults to $EXCISE_ORACLE_CACHE_DIR or the system temp cache.",
        };
        ...
        var exciseRenderCacheDirOption = new Option<DirectoryInfo?>("--excise-render-cache-dir")
        {
            Description = "Directory for cached excise-rendered PNGs. Disabled unless explicitly set.",
        };
        ...
        var incrementalOutputOption = new Option<bool>("--incremental-output")
        {
            Description = "Rewrite the main output JSON with completed page results on each heartbeat.",
            DefaultValueFactory = _ => false,
        };
        var largePdfShardPagesOption = new Option<int>("--large-pdf-shard-pages")
        {
            Description = "When a page manifest selects many pages from one PDF, split it into page-count shards so large PDFs do not monopolize one worker. 0 disables sharding.",
            DefaultValueFactory = _ => 0,
        };

        var command = new Command("corpus-scan",
            "Render corpus PDFs with excise + reference oracles, compute pixel-diff, write JSON report")
        {
        ...
        command.SetAction(parseResult =>
        {
            ...
            var incrementalOutput = parseResult.GetValue(incrementalOutputOption);
            ...
            if (!Excise.Rendering.Differential.MutoolReferenceRenderer.IsAvailable)
            {
    ...
    internal static bool RunCorpusScan(
        ...
        DirectoryInfo? exciseRenderCacheDir = null,
        int progressIntervalSeconds = 30,
        string? progressOutputPath = null,
        bool incrementalOutput = false,
        int largePdfShardPages = 0)
    {
        var pdfs = DiscoverCorpusPdfs(corpusDir, chunkIndex, chunkTotal, pageManifest?.Keys);
        var workItems = ExpandCorpusWorkItems(pdfs, pageManifest, largePdfShardPages);
        var oracleCache = CreateOracleRenderCache(oracleCacheDir);
        var exciseRenderCache = CreateExciseRenderCache(exciseRenderCacheDir);
        var shardedPdfCount = workItems.Count - pdfs.Count;

        Console.Out.WriteLine(
            $"chunk {chunkIndex + 1}/{chunkTotal}: scanning {workItems.Count} work items from {pdfs.Count} PDFs in {corpusDir} " +
            ...
            $"large-pdf-shards={(largePdfShardPages <= 0 ? "off" : $"{largePdfShardPages} pages, +{shardedPdfCount} work items")}, " +
            ...
            $"excise-cache={(exciseRenderCache is null ? "off" : exciseRenderCache.CacheDirectory)})");
        ...
        var resolvedProgressOutputPath = progressIntervalSeconds <= 0
            ...
            : string.IsNullOrWhiteSpace(progressOutputPath)
                ...
                : progressOutputPath;
        var resolvedIncrementalOutputPath = progressIntervalSeconds <= 0 || !incrementalOutput
            ? null
            : outputPath;
        var progressReporter = StartCorpusScanProgressReporter(
            workItems.Count,
            entries,
            activeProgress,
        ...
        try
        {
            Parallel.ForEach(workItems, po, corpusPdf =>
            {
                ...
                var progressKey = corpusPdf.WorkItemKey ?? rel;
                var progress = new CorpusScanProgress(progressKey);
                activeProgress[progressKey] = progress;
                try
                {
                    IReadOnlySet<int>? selectedPages = corpusPdf.SelectedPages;
                    ...
                    // Probe the oracles on any page excise refused, BEFORE it is
                    // published (#882). The failure paths inside ScanOnePdf all
                    // return before the comparison block, so without this every
                    // refusal carries null oracle statuses and the agreement
                    // classifier cannot tell "nobody could render it" from
                    // "only excise could not".
                    ...
                    {
                        // The TIMEOUT entry is synthesized here rather than by
                        // ScanOnePdf, so it needs the same probe (#882) — it is
                        // the page that exposed this whole gap.
                ...
                {
                    activeProgress.TryRemove(progressKey, out _);
                }
            });
        }
        finally
        ...
        var exciseRenderCacheReport = exciseRenderCache?.CreateReport() ?? CorpusRenderCacheReport.CreateDisabled();
        if (exciseRenderCacheReport.enabled)
        {
            Console.Out.WriteLine(
                $"  excise render cache: {exciseRenderCacheReport.hits} hits, {exciseRenderCacheReport.misses} misses, " +
                $"{exciseRenderCacheReport.writes} writes, {exciseRenderCacheReport.errors} errors");
        }
        ...
        var report = new
        {
            ...
            largePdfShardPages = largePdfShardPages > 0 ? largePdfShardPages : (int?)null,
            peakRssBytes = peakBytes,
            oracleCache = oracleCacheReport,
            exciseRenderCache = exciseRenderCacheReport,
    ...
    private static IReadOnlyList<CorpusPdf> ExpandCorpusWorkItems(
        IReadOnlyList<CorpusPdf> pdfs,
        IReadOnlyDictionary<string, IReadOnlySet<int>>? pageManifest,
        int largePdfShardPages)
    {
        if (pageManifest is null || largePdfShardPages <= 0)
            return pdfs;

        var workItems = new List<CorpusPdf>(pdfs.Count);
        foreach (var pdf in pdfs)
        {
            if (!pageManifest.TryGetValue(pdf.RelativePath, out var selectedPages) ||
                selectedPages.Count <= largePdfShardPages)
            {
                workItems.Add(pdf);
                continue;
            }

            var orderedPages = selectedPages.OrderBy(page => page).ToArray();
            for (var offset = 0; offset < orderedPages.Length; offset += largePdfShardPages)
            {
                var shardPages = orderedPages
                    .Skip(offset)
                    .Take(largePdfShardPages)
                    .ToHashSet();
                var firstPage = orderedPages[offset];
                var lastPage = orderedPages[Math.Min(offset + largePdfShardPages, orderedPages.Length) - 1];
                workItems.Add(pdf with
                {
                    SelectedPages = shardPages,
                    WorkItemKey = $"{pdf.RelativePath} pages {firstPage}-{lastPage}",
                });
            }
        }
    ...
    internal readonly record struct CorpusPdf(
        string FullPath,
        string RelativePath,
        ...
        ExciseRenderCache? exciseRenderCache = null)
    {
        ...
        SkiaSharp.SKBitmap? exciseBmp = null;
        ...
        try
        {
            try
            {
                progress?.Update("render", pageNumber, $"excise render page {pageNumber}/{doc.PageCount}");
                renderDiagnostics.Clear();
                var exciseOutcome = RenderExciseWithCache(
                    exciseRenderCache, pdfPath, pageNumber, dpi, userPassword,
                    () =>
                    {
                ...
                exciseBmp = exciseOutcome.Result.Bitmap;
                entry.renderMs = exciseOutcome.Result.ElapsedMs;
                ApplyExciseCacheFields(entry, exciseOutcome);
                ApplyRenderDiagnostics(entry, exciseOutcome.Result.Diagnostics);
            }
            catch (Exception ex)
            {
                if (ex is Excise.Rendering.RenderResourceLimitException
                    && TryComputeResourceSafeDpi(doc.GetPage(pageNumber), dpi, out var fallbackDpi))
                {
                    progress?.Update("render", pageNumber,
                        $"excise render page {pageNumber}/{doc.PageCount} at fallback {fallbackDpi} DPI");
                    try
                    {
                        renderDiagnostics.Clear();
                        var fallbackOutcome = RenderExciseWithCache(
                            exciseRenderCache, pdfPath, pageNumber, fallbackDpi, userPassword,
                            () =>
                            {
                        ...
                        exciseBmp = fallbackOutcome.Result.Bitmap;
                        entry.renderMs = fallbackOutcome.Result.ElapsedMs;
                        ApplyExciseCacheFields(entry, fallbackOutcome);
            ...
            if (mutoolBmp != null)
            {
                progress?.Update("compare", pageNumber, $"compare excise vs mutool page {pageNumber}/{doc.PageCount}");
                var (a, b) = MatchAndCompare(exciseBmp, mutoolBmp);
                mutoolMetrics = (a, b);
            ...
            if (cairoBmp != null)
            {
                progress?.Update("compare", pageNumber, $"compare excise vs pdftocairo page {pageNumber}/{doc.PageCount}");
                var (a, b) = MatchAndCompare(exciseBmp, cairoBmp);
                cairoMetrics = (a, b);
            ...
            if (shouldEscalate && extraOracles.HasFlag(CorpusExtraOracles.Ghostscript))
            {
                ...
                {
                    progress?.Update("compare", pageNumber, $"compare excise vs ghostscript page {pageNumber}/{doc.PageCount}");
                    var (a, b) = MatchAndCompare(exciseBmp, ghostscriptBmp);
            ...
            if (shouldEscalate && extraOracles.HasFlag(CorpusExtraOracles.PdfBox))
            {
                ...
                if (pdfboxBmp != null)
                {
                    progress?.Update("compare", pageNumber, $"compare excise vs pdfbox page {pageNumber}/{doc.PageCount}");
                    var (a, b) = MatchAndCompare(exciseBmp, pdfboxBmp);
                    pdfboxMetrics = (a, b);
            ...
            if (bestReference != null)
                ApplyCorpusVisualDiffClassification(entry, exciseBmp, bestReference);
    ...
    private static ExciseRenderOutcome RenderExciseWithCache(
        ExciseRenderCache? cache,
        string pdfPath,
        int pageNumber,
        int dpi,
        string? userPassword,
        Func<ExciseRenderResult> render)
    {
        return cache?.GetOrRender(pdfPath, pageNumber, dpi, userPassword, render)
               ?? new ExciseRenderOutcome(render(), CacheEnabled: false, CacheHit: false,
                   CachedRenderMs: null, CachedStatus: null, CachedErrorMessage: null);
    }

    private static void ApplyExciseCacheFields(CorpusScanEntry entry, ExciseRenderOutcome outcome)
    {
        if (!outcome.CacheEnabled)
            return;

        entry.exciseCacheHit = outcome.CacheHit;
        entry.exciseCachedRenderMs = outcome.CachedRenderMs;
        entry.exciseCachedStatus = outcome.CachedStatus;
        entry.exciseCachedError = TruncNullable(outcome.CachedErrorMessage, 200);
    }
    ...
    private static ExciseRenderCache? CreateExciseRenderCache(DirectoryInfo? cacheDir)
    {
        if (cacheDir == null)
            return null;
        ...
        try
        {
            return new ExciseRenderCache(cacheDir.FullName);
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"warning: excise render cache disabled: {ex.Message}");
            return null;
        }
    }
    ...
    private sealed class ExciseRenderCache
    {
        private const string CacheVersion = "excise-v1";
        ...
        public ExciseRenderCache(string cacheDirectory)
        {
        ...
        public ExciseRenderOutcome GetOrRender(
            string pdfPath,
            int pageNumber,
            int dpi,
            string? userPassword,
            Func<ExciseRenderResult> render)
        {
            ...
            lock (gate)
            {
                if (TryDecode(cachePath, out var cachedBitmap, out var elapsedMs, out var metadata))
                {
                    ...
                    return new ExciseRenderOutcome(
                        new ExciseRenderResult(
                            cachedBitmap,
                            metadata?.status ?? "OK",
                ...
                return new ExciseRenderOutcome(result, CacheEnabled: true, CacheHit: false,
                    CachedRenderMs: null, CachedStatus: null, CachedErrorMessage: null);
            }
        }
        ...
        private bool TryDecode(
            ...
            out SKBitmap? bitmap,
            out long elapsedMs,
            out ExciseRenderCacheMetadata? metadata)
        {
            ...
            ExciseRenderResult result)
        {
            ...
            try
            {
                ...
                var metadata = new ExciseRenderCacheMetadata
                {
        ...
        private ExciseRenderCacheMetadata? TryReadMetadata(string metadataPath)
        {
            ...
            try
            {
                return System.Text.Json.JsonSerializer.Deserialize<ExciseRenderCacheMetadata>(
    ...
    private sealed record ExciseRenderResult(
        SKBitmap? Bitmap,
        string Status,
        string? ErrorMessage,
        long ElapsedMs,
    ...
    private sealed record ExciseRenderOutcome(
        ExciseRenderResult Result,
        bool CacheEnabled,
        bool CacheHit,
        long? CachedRenderMs,
    ...
    private sealed class ExciseRenderCacheMetadata
    {
        public string cacheVersion { get; set; } = "";
    ...
    internal sealed class CorpusScanPriorityEntry
    {
        ...
        public double? exciseReferenceCenterScore { get; set; }
        public double? oracleMeanCenterScore { get; set; }
        public int? exciseReferenceCenterRank { get; set; }
    }
        ...
        IReadOnlySet<int>? selectedPages = null)
    {
        ...
        // Each page always attempts excise, mutool, and pdftocairo. If the
    ...
    /// </summary>
    /// <summary>
    /// Number of tiles per axis for the ink-locality comparison (#883).
    /// 32 gives ~1500 tiles on a letter page — small enough that a word lands
    /// inside one or two, large enough that antialiasing never fills one alone.
    /// </summary>
    /// <summary>
    /// Ask the primary oracles whether THEY can render a page excise refused
    /// (#882).
    ///
    /// Without this the scan never invoked an oracle on any failure path —
    /// MALFORMED_PDF, DECODE_ERROR, EMPTY_DOC and the encryption statuses all
    /// returned before the comparison block. Every refusal therefore carried
    /// five null oracle statuses, and null is indistinguishable from "tried and
    /// failed" unless you look for it. The agreement classifier read that as
    /// "nobody else could render it either" and counted 64 of 65 PDFium
    /// refusals as correct behaviour on evidence that was never gathered.
    ///
    /// That is the project's own rule being broken in the harness: excise
    /// refusing, and excise being the only thing consulted about whether
    /// refusing was right.
    ///
    /// Only the two primaries run. They are independent engines (MuPDF and
    /// Poppler), which is enough to tell "corroborated refusal" from
    /// "excise-only failure", and a refusal is cheap for them too — these are
    /// files that fail fast.
    /// </summary>
    /// <summary>
    /// True when this entry records a refusal that no oracle has been asked
    /// about. `renderMs == null` means excise produced no bitmap; null primary
    /// statuses mean the comparison block was never reached.
    /// </summary>
    private static bool NeedsRefusalProbe(CorpusScanEntry e)
        => e.renderMs == null && e.mutoolStatus == null && e.cairoStatus == null;

    private static void ProbeOraclesForRefusal(
        CorpusScanEntry entry,
        string pdfPath,
        int pageNumber,
        int dpi,
        int oracleTimeoutMs,
        string? userPassword,
        OracleRenderCache? oracleCache)
    {
        // Page 0 is the document-level failure sentinel; probe page 1 instead.
        var probePage = pageNumber <= 0 ? 1 : pageNumber;
        try
        {
            ...
            entry.mutoolMs = mutool.Result.ElapsedMs;
            entry.mutoolError = TruncNullable(mutool.Result.ErrorMessage, 200);
            mutool.Result.Bitmap?.Dispose();
            ...
            entry.cairoMs = cairo.Result.ElapsedMs;
            entry.cairoError = TruncNullable(cairo.Result.ErrorMessage, 200);
            cairo.Result.Bitmap?.Dispose();
        }
        catch (Exception ex)
        {
            // Probing is best-effort: it exists to add evidence to a page that
            // already failed, and must never turn that into a different error.
            entry.diagnostic = AppendDiagnostic(entry.diagnostic,
    ...
    /// </summary>
    private const double InkTileThreshold = 0.005;

    /// <summary>
    /// Compare WHERE the ink is, not just how much (#883).
    ///
    /// diffFraction and ink coverage are both whole-page aggregates, so content
    /// that is small but meaningful is invisible to them. The worked case:
    /// bug_631912.pdf reported PASS at diffFraction 0.0012 while excise rendered
    /// the page BLANK and every oracle rendered the word "Test" — 760 dark
    /// pixels out of ~1.3 million, 0.06% of the page. No threshold on a
    /// page-wide average can separate that from antialiasing noise, because it
    /// IS smaller than antialiasing noise.
    ///
    /// Locality separates them. Tile the page; a tile where the oracle has real
    /// ink and excise has essentially none means content was dropped, however
    /// little of the page it occupies. Antialiasing cannot produce that — it
    /// perturbs edges of ink that is present in both.
    ///
    /// For a redaction tool the missing direction is the dangerous one: a
    ...
    private static bool IsInkPixel(SkiaSharp.SKColor p)
        => p.Alpha > 16 && (p.Red < 200 || p.Green < 200 || p.Blue < 200);
        ...
        SkiaSharp.SKBitmap exciseBmp, SkiaSharp.SKBitmap reference)
    {
        ...
        try
        {
            var report = Excise.Rendering.Differential.DifferentialMetrics.Compare(probe, reference);
        ...
        double maxMae)
    {
        var rendered = renderers
            ...
            .ToArray();
        if (rendered.Length < 3 || rendered.All(item => item.Name != "excise"))
            return;
        ...
        var exciseReferencePairs = pairs
            .Where(pair => pair.a == "excise" ^ pair.b == "excise")
            .ToArray();
        if (exciseReferencePairs.Length == 0)
            return;

        entry.exciseReferenceCenterScore = exciseReferencePairs.Average(pair => pair.score);

        var oracleNames = rendered
            .Select(item => item.Name)
            .Where(name => name != "excise")
            .ToArray();
        ...
        entry.exciseReferenceCenterRank =
            1 + sortedOracleScores.Count(score => score.score < entry.exciseReferenceCenterScore);
        ...
        double maxMae)
    {
        ...
        if (entry.exciseReferenceCenterScore is not { } exciseCenter ||
        ...
        SkiaSharp.SKBitmap reference)
    {
        ...
        if (pixels > VisualVectorMaxPixelCount)
        {
            ...
            var scaledProbe = Excise.Rendering.Differential.DifferentialMetrics
                .ResizeMatch(probe, targetWidth, targetHeight);
            var scaledReference = Excise.Rendering.Differential.DifferentialMetrics
    ...
    private static void ApplyRenderDiagnostics(CorpusScanEntry entry, IReadOnlyList<string> diagnostics)
    {
        ...
        entry.diagnostic = AppendDiagnostic(
            entry.diagnostic,
            "excise=" + string.Join("; ", diagnostics.Select(d => Trunc(d, 200))));
    }
        ...
        Stopwatch pageStopwatch)
    {
        ...
        entry.diagnostic = AppendDiagnostic(
            entry.diagnostic,
            "Skipped reference oracles because excise recovered malformed page content.");
        entry.elapsedMs = pageStopwatch.ElapsedMilliseconds;
        return true;
    }
    ...
    private static IDisposable? StartCorpusScanProgressReporter(
        ...
        CorpusPageMode pageMode,
        CorpusExtraOracles extraOracles,
        string? incrementalOutputPath,
        string corpusDir,
        IReadOnlyDictionary<string, IReadOnlySet<int>>? pageManifest,
        IReadOnlyDictionary<string, string>? passwordManifest,
        IReadOnlyDictionary<CorpusPageKey, CorpusExpectedOutcome>? expectationManifest)
    {
        ...
        IReadOnlyDictionary<CorpusPageKey, CorpusExpectedOutcome>? expectationManifest)
    {
        ...
        Console.Out.WriteLine(
            $"progress {processedPdfs}/{totalPdfs} work items, {completedEntries.Length} page results, " +
        ...
        var report = new
        {
            ...
            totalWorkItems = totalPdfs,
            processedWorkItems = processedPdfs,
        ...
        if (!string.IsNullOrWhiteSpace(incrementalOutputPath))
        {
            var sortedEntries = completedEntries
            ...
            var partialReport = new
            {
                ...
                expectationManifest = expectationManifest is null ? null : new { entries = expectationManifest.Count },
                extraOracles = ExtraOraclesName(extraOracles),
                counts = CountBy(sortedEntries.Select(entry => entry.status)),
                resultCounts = CountBy(sortedEntries.Select(entry => entry.resultStatus)),
                resultCategoryCounts = CountBy(sortedEntries.Select(GetCorpusResultCategory)),
                summary = BuildCorpusScanSummary(sortedEntries),
            ...
            WriteJsonAtomically(incrementalOutputPath, partialReport);
        }
    }
    ...
    {
        if (ex is Excise.Rendering.InvalidPageGeometryException)
    ...
    private static CorpusScanPriorityEntry ToPriorityEntry(CorpusScanEntry entry)
        => new()
        {
            ...
            exciseReferenceCenterScore = entry.exciseReferenceCenterScore,
            oracleMeanCenterScore = entry.oracleMeanCenterScore,
            exciseReferenceCenterRank = entry.exciseReferenceCenterRank,
        };
    ...
    internal sealed class CorpusScanEntry
    {
        ...
        public bool? exciseCacheHit { get; set; }
        ...
        public long? exciseCachedRenderMs { get; set; }
        ...
        public string? exciseCachedStatus { get; set; }
        ...
        public string? exciseCachedError { get; set; }
