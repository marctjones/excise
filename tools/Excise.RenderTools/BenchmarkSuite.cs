...
partial class Program
{
    ...
    static Command CreateBenchmarkSuiteCommand()
    {
        ...
        var maxExciseRenderMsOption = new Option<double>("--max-excise-render-ms")
        {
            Description = "Regression gate: maximum average excise render time per page.",
            DefaultValueFactory = _ => 2_500,
        };
        var maxExciseParseMsOption = new Option<double>("--max-excise-parse-ms")
        {
            Description = "Regression gate: maximum average excise open/parse time per document.",
            DefaultValueFactory = _ => 750,
        };
        var minReferencePassRateOption = new Option<double>("--min-reference-pass-rate")
        {
            Description = "Regression gate: minimum excise-vs-reference page pass rate when at least one reference renderer is available.",
            DefaultValueFactory = _ => 0.60,
        };
        var includeCliRenderOption = new Option<bool>("--include-cli-render")
        {
            Description = "Also render each page through the excise CLI subprocess and compare it to the in-process renderer.",
            DefaultValueFactory = _ => false,
        };
            ...
            "Run speed, fidelity, text, robustness, and redaction-completeness benchmark reports")
        {
            ...
            maxExciseRenderMsOption,
            maxExciseParseMsOption,
        ...
        command.SetAction(parseResult =>
        {
            ...
            var maxExciseRenderMs = parseResult.GetValue(maxExciseRenderMsOption);
            var maxExciseParseMs = parseResult.GetValue(maxExciseParseMsOption);
        ...
        bool includeCliRender = false)
    {
        ...
        return new BenchmarkSuiteReport
        {
            ...
            {
                policy = "Copyleft or AGPL/GPL reference renderers are invoked only as external CLI subprocesses; no MuPDF, Poppler, Ghostscript, or PDFBox libraries are referenced by excise.",
                shippableGraph = "The benchmark suite lives under tools/Excise.RenderTools and scripts/run-benchmarks.sh, outside the desktop app dependency graph.",
                permissiveInProcess = "excise's own parser/renderer run in-process. Optional permissive library benchmarks remain isolated to dev tooling.",
            },
        ...
        bool includeCliRender)
    {
        ...
        try
        {
            ...
            exciseBitmap = renderer.RenderPage(doc.GetPage(pageNumber), new RenderOptions { Dpi = dpi });
            renderSw.Stop();

            var references = new List<BenchmarkReferenceResult>();
            foreach (var oracle in oracles)
            {
                var reference = BenchmarkReference(oracle, input.Path, pageNumber, dpi, timeoutMs, exciseBitmap);
                references.Add(reference);
            }

            var cliRender = includeCliRender
                ? BenchmarkCliRender(input.Path, pageNumber, dpi, timeoutMs, exciseBitmap)
                : null;
            ...
            return new BenchmarkPageResult
            {
                ...
                source = input.Source,
                parseMs = parseMs,
                exciseRenderMs = renderSw.ElapsedMilliseconds,
                exciseTextExtractMs = textSw.ElapsedMilliseconds,
                exciseTextLength = text.Length,
                exciseWidth = exciseBitmap.Width,
                exciseHeight = exciseBitmap.Height,
    ...
    private static BenchmarkCliRenderResult BenchmarkCliRender(
        ...
        int dpi,
        int timeoutMs,
        SKBitmap exciseBitmap)
    {
        var outputPath = Path.Combine(
            Path.GetTempPath(),
            $"excise-cli-benchmark-{Environment.ProcessId}-{Guid.NewGuid():N}.png");
        var invocation = ResolveExciseCliInvocation();
        if (invocation is null)
        {
            return new BenchmarkCliRenderResult
            {
                ...
                status = "MISSING",
                error = "Could not locate Excise.Cli project or built assembly.",
            };
        }
    ...
    private static string? FindRepositoryRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(current.FullName, "excise.sln")) &&
                File.Exists(Path.Combine(current.FullName, "Excise.Cli", "Excise.Cli.csproj")))
            {
        ...
        SKBitmap exciseBitmap)
    {
        ...
        using var normalizedReference = DifferentialMetrics.ResizeMatch(referenceBitmap, exciseBitmap.Width, exciseBitmap.Height);
        var diff = DifferentialMetrics.Compare(exciseBitmap, normalizedReference);
        var rmse = CalculateRmse(exciseBitmap, normalizedReference);
        var ssim = CalculateLuminanceSsim(exciseBitmap, normalizedReference);
        return new BenchmarkReferenceResult
        {
            name = oracle.Name,
            kind = "external-cli",
    ...
    {
        var exciseRender = pages.Select(p => p.exciseRenderMs).Where(v => v >= 0).OrderBy(v => v).ToArray();
        var exciseText = pages.Select(p => p.exciseTextExtractMs).Where(v => v >= 0).OrderBy(v => v).ToArray();
        ...
        return new BenchmarkSummary
        {
            ...
            exciseParseAverageMs = Average(parse),
            exciseParseP95Ms = PercentileDouble(parse, 0.95),
            exciseRenderAverageMs = Average(exciseRender),
            exciseRenderP50Ms = PercentileDouble(exciseRender, 0.50),
            exciseRenderP95Ms = PercentileDouble(exciseRender, 0.95),
            exciseTextExtractAverageMs = Average(exciseText),
    ...
    private static BenchmarkRegressionGate EvaluateBenchmarkGate(BenchmarkSummary summary, BenchmarkGateConfig config)
    {
        var checks = new List<BenchmarkGateCheck>
        {
            new()
            {
                name = "excise-render-average",
                actual = summary.exciseRenderAverageMs,
                threshold = config.MaxExciseRenderMs,
                passed = summary.exciseRenderAverageMs <= config.MaxExciseRenderMs,
                unit = "ms",
            },
            new()
            {
                name = "excise-parse-average",
                actual = summary.exciseParseAverageMs,
                threshold = config.MaxExciseParseMs,
                passed = summary.exciseParseAverageMs <= config.MaxExciseParseMs,
                unit = "ms",
            },
            new()
            {
        ...
        if (summary.cliRenderPassRate.HasValue)
        {
            checks.Add(new BenchmarkGateCheck
            {
                name = "excise-cli-render-pass-rate",
        ...
        BenchmarkRedactionResult redaction)
    {
        ...
        AddBenchmarkHotPath(
            ...
            "Excise.Rendering page rasterization",
            pages.Select(p => p.exciseRenderMs),
            "excise-owned",
            "#598 #599");
        AddBenchmarkHotPath(
            buckets,
            "text.extract-search-input",
            "Excise.Core text extraction and text-path preparation",
            pages.Select(p => p.exciseTextExtractMs),
            "excise-owned",
            "#600");
        AddBenchmarkHotPath(
            buckets,
            "parser.document-open",
            "Excise.Core document open/parse time",
            pages
        ...
        AddBenchmarkHotPath(
            ...
            "Excise.Cli render command subprocess, PNG write, and output comparison",
            pages
        ...
        AddBenchmarkHotPath(
            ...
            "External reference renderer subprocess time; reported for comparison, not optimized by excise",
            pages.SelectMany(p => p.references).Where(r => r.elapsedMs.HasValue).Select(r => r.elapsedMs!.Value),
            "external-reference",
            "#597");

        return buckets
            .OrderByDescending(bucket => bucket.scope == "excise-owned" || bucket.scope == "excise-owned-security-critical")
    ...
    private static IReadOnlyList<BenchmarkInput> ResolveBenchmarkInputs(string outputDir, string? corpusDir, int pageLimit)
    {
        ...
        var syntheticPath = Path.Combine(syntheticDir, "excise-benchmark-synthetic.pdf");
        File.WriteAllBytes(syntheticPath, CreateSamplePdf());
        return new[]
        {
            new BenchmarkInput(syntheticPath, "synthetic/excise-benchmark-synthetic.pdf", "synthetic"),
        };
    }
        ...
        bool includeCliRender)
    {
        ...
        return new[]
        {
            Tool("excise", "in-process", true, true, "MIT project code"),
            Tool("excise-cli", "external-subprocess", ResolveExciseCliInvocation() is not null, includeCliRender, "excise CLI invoked as a subprocess to test the shipped command route"),
    ...
    private static string BuildBenchmarkCsv(BenchmarkSuiteReport report)
    {
        ...
        sb.AppendLine("path,page,status,exciseRenderMs,exciseTextExtractMs,cliRenderStatus,cliRenderMs,cliRenderPass,cliDiffFraction,referenceCount,passingReferenceCount,bestReference,bestDiffFraction,bestMae,bestRmse,bestSsim");
        foreach (var page in report.pages)
        {
            ...
            sb.AppendLine(string.Join(",",
                ...
                Csv(page.status),
                page.exciseRenderMs.ToString(CultureInfo.InvariantCulture),
                page.exciseTextExtractMs.ToString(CultureInfo.InvariantCulture),
    ...
    {
        var sb = new StringBuilder();
        sb.AppendLine("# excise Benchmark Report");
        sb.AppendLine();
        ...
        sb.AppendLine($"- excise render average: `{report.summary.exciseRenderAverageMs:F1} ms`");
        sb.AppendLine($"- excise render p95: `{report.summary.exciseRenderP95Ms:F1} ms`");
        if (report.summary.cliRenderPassRate.HasValue)
        {
            sb.AppendLine($"- excise CLI render average: `{report.summary.cliRenderAverageMs:F1} ms`");
            sb.AppendLine($"- excise CLI render pass rate: `{report.summary.cliRenderPassRate.Value:P1}`");
        }
        ...
        sb.AppendLine();
        sb.AppendLine("| PDF | Page | Status | excise render ms | CLI | References |");
        sb.AppendLine("|---|---:|---|---:|---|---:|");
        foreach (var page in report.pages)
        {
            ...
            sb.AppendLine($"| `{page.path}` | {page.pageNumber} | {page.status} | {page.exciseRenderMs} | {cli} | {page.passingReferenceCount}/{page.referenceCount} |");
        }
    ...
    private static void PrintBenchmarkSuiteSummary(BenchmarkSuiteReport report)
    {
        Console.Out.WriteLine(
            ...
            $"excise render avg {report.summary.exciseRenderAverageMs:F1}ms, " +
        ...
        var topExciseHotPath = report.hotPaths.FirstOrDefault(h => h.scope.StartsWith("excise-owned", StringComparison.Ordinal));
        if (topExciseHotPath is not null)
            Console.Out.WriteLine($"Top excise-owned hot path: {topExciseHotPath.name} ({topExciseHotPath.totalMs}ms total)");
    ...
    internal sealed record BenchmarkGateConfig(
        double MaxExciseRenderMs,
        double MaxExciseParseMs,
    ...
    internal sealed class BenchmarkSummary
    {
        ...
        public double exciseParseAverageMs { get; set; }
        public double exciseParseP95Ms { get; set; }
        public double exciseRenderAverageMs { get; set; }
        public double exciseRenderP50Ms { get; set; }
        public double exciseRenderP95Ms { get; set; }
        public double exciseTextExtractAverageMs { get; set; }
    ...
    internal sealed class BenchmarkPageResult
    {
        ...
        public long exciseRenderMs { get; set; }
        public long exciseTextExtractMs { get; set; }
        public int exciseTextLength { get; set; }
        public int? exciseWidth { get; set; }
        public int? exciseHeight { get; set; }
