...
///
/// Baseline workflow:
///   1. First run with EXCISE_UPDATE_BASELINES=1 writes the baseline next to the
...
public class PdfViewerHeadlessRenderTests
{
    ...
    public async Task PdfViewer_RenderingQualitySuite_DisplayBitmapsMatchRenderer(int rowShardIndex)
    {
        ...
        var pageLimit = ParseOptionalPositiveEnvironmentInt("EXCISE_GUI_DISPLAY_PAGE_LIMIT");
        var caseFilter = Environment.GetEnvironmentVariable("EXCISE_GUI_DISPLAY_CASE_FILTER");
        ...
        // look the same.
        var softDeadlineMs = ParsePositiveEnvironmentInt("EXCISE_GUI_DISPLAY_DEADLINE_MS", defaultValue: 1_500_000);
        var timedOut = false;
        ...
        timedOut.Should().BeFalse(
            ...
            "EXCISE_GUI_DISPLAY_SHARD_COUNT / EXCISE_GUI_DISPLAY_SHARD_INDEX. Raise the budget " +
            "with EXCISE_GUI_DISPLAY_DEADLINE_MS if the machine is genuinely slower. " +
    ...
    {
        var pdfBytes = CreatePdfWithContent("BT /F1 24 Tf 100 700 Td (Hello, World!) Tj ET");
        using var doc = Excise.Core.Document.PdfDocument.Open(pdfBytes);
    ...
    private void AssertMatchesBaseline(SKBitmap actual, string testName, double maxDifference)
    {
        ...
        if (Environment.GetEnvironmentVariable("EXCISE_UPDATE_BASELINES") == "1")
        {
        ...
        if (!File.Exists(committedBaseline))
        {
            throw new FileNotFoundException(
                ...
                $"Run with EXCISE_UPDATE_BASELINES=1 to generate, verify the PNG, then commit.");
        }
    ...
    private static string FindSourceBaselinePath(string testName)
    {
        ...
        if (dir == null)
            throw new InvalidOperationException("Could not locate Excise.App.Tests project root.");
    ...
    /// from Excise.Rendering.Tests so this test is self-contained and doesn't
