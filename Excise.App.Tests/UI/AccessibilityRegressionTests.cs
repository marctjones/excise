...
public class AccessibilityRegressionTests
{
    ...
    public void SearchControls_StatusBarAndViewer_ExposeKeyboardAccessibleNames()
    {
        ...
        var viewer = window.FindControl<Excise.Avalonia.Controls.PdfViewerControl>("PdfViewerControl");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"excise-accessibility-keyboard-{Guid.NewGuid():N}.pdf");
        ...
        try
        {
            ...
            await window.PressKeyAsync(global::Avalonia.Input.Key.F, global::Avalonia.Input.RawInputModifiers.Control);
            await KeyboardTestHelpers.FlushDispatcherAsync();
            ...
            await window.PressKeyAsync(global::Avalonia.Input.Key.PageDown);
            await KeyboardTestHelpers.FlushDispatcherAsync();
