...
public class GuiFullResponsivenessCoverageTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"excise-long-scroll-{Guid.NewGuid():N}.pdf");
    ...
    {
        var inputPath = Path.Combine(Path.GetTempPath(), $"excise-broad-gui-{Guid.NewGuid():N}.pdf");
        var outputPath = Path.Combine(Path.GetTempPath(), $"excise-broad-gui-output-{Guid.NewGuid():N}.pdf");
