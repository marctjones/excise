...
namespace Excise.App.Tests.Documentation;
...
public class DocumentationClaimTests
{
    ...
    [Theory]
    [InlineData("README.md", "page organization", "Excise.App/Views/MainWindow.axaml", "Insert Pages _Before Current")]
    [InlineData("README.md", "move current or selected pages earlier/later", "Excise.App/ViewModels/MainWindowViewModel.Commands.cs", "MoveCurrentPageLaterCommand")]
    [InlineData("README.md", "selected pages", "Excise.App/ViewModels/MainWindowViewModel.Commands.cs", "ExtractSelectedPagesCommand")]
    [InlineData("README.md", "selected pages", "Excise.App/ViewModels/MainWindowViewModel.Commands.cs", "MoveSelectedPagesLaterCommand")]
    [InlineData("README.md", "AddTextAnnotation", "Excise.Core/Document/PdfAnnotationAuthoring.cs", "AddTextAnnotation")]
    ...
    [InlineData("README.md", "PublicApiApprovalTests", "Excise.Core.Tests/Authoring/PublicApiApprovalTests.cs", "APPROVE_PUBLIC_API")]
    ...
    public void ReleaseChecklist_IncludesDocumentationAndValidationGates()
    {
        ...
        checklist.Should().Contain("dotnet build excise.sln --no-restore");
        checklist.Should().Contain("dotnet test excise.sln --no-build");
    ...
    public void AccessibilityReleaseDocs_MapToGateAndCommandMetadata()
    {
        ...
        var commandRegistry = Read("Excise.Core/Automation/PdfCommandRegistry.cs");
        var commandAccessibility = Read("Excise.App/Automation/CommandAccessibility.cs");
    ...
    public void AutomationReleaseDocs_MapToCliBatchGateAndPlatformExamples()
    {
        ...
        var program = Read("Excise.Cli/Program.cs");
        var batch = Read("Excise.Cli/AutomationBatch.cs");
        var registry = Read("Excise.Core/Automation/PdfCommandRegistry.cs");
        var macos = Read("automation-scripts/macos/render-page.applescript");
        var powershell = Read("automation-scripts/windows/Excise.Automation.psm1");
        var linux = Read("automation-scripts/linux/excise-automation.sh");
        var dbus = Read("automation-scripts/linux/gnome-dbus-evaluation.md");

        readme.Should().Contain("excise batch");
        ...
        powershell.Should().Contain("Invoke-ExciseBatch");
        powershell.Should().Contain("Invoke-ExciseRedaction");
    ...
    public void UxIconAuditDocs_MapToReleaseSmokeAndVisualPolishTests()
    {
        ...
        var visualPolishTests = Read("Excise.App.Tests/UI/VisualPolishAuditTests.cs");
        var mainWindow = Read("Excise.App/Views/MainWindow.axaml");
        var icons = Read("Excise.App/Styles/Icons.axaml");
    ...
    public void BenchmarkScript_UsesExistingRenderToolsHotspotCommands()
    {
        ...
        var benchmarkSuite = Read("tools/Excise.RenderTools/BenchmarkSuite.cs");

        script.Should().Contain("benchmark-suite");
        script.Should().Contain("benchmarkdotnet");
        script.Should().Contain("Excise.Benchmarks/Excise.Benchmarks.csproj");
        script.Should().Contain("tools/Excise.RenderTools/Excise.RenderTools.csproj");
        ...
        releaseSmoke.Should().Contain("automation,ux,benchmark,perf-budget,aot,pdf20,corpus-resilience,adversarial-extraction,copy-parity,tests");
    ...
    public void FileAssociationDocs_MapToPackagingAndStartupHandlers()
    {
        ...
        var windowsInstaller = Read("packaging/windows/excise.iss");
        var appStartup = Read("Excise.App/App.axaml.cs");

        readme.Should().Contain("Using excise as a PDF reader on macOS");
        readme.Should().Contain("Using excise as a PDF reader on Windows");
        readme.Should().Contain("Associate excise with .pdf files");
