// Workaround for upstream global::Avalonia.Headless.XUnit 12.0.2 bug — the stock
...
// global::Avalonia.Headless.XUnit) but routes the dispatch through DispatchCore
// with captureExecutionContext: true via reflection — propagating
// TestContext.Current correctly onto the dispatcher thread.
//
// Tracking: see issue #337. When upstream global::Avalonia.Headless.XUnit ships a
    ...
    : XunitTestRunnerBase<FixedAvaloniaTestRunnerContext, IXunitTest>
{
    ...
    private static readonly MethodInfo DispatchCoreMethod =
        ...
        ?? throw new InvalidOperationException(
            "HeadlessUnitTestSession.DispatchCore not found — global::Avalonia.Headless internals " +
...
internal static class HeadlessWindowTracker
{
    ...
    public static void Ensure()
    {
        ...
        Window.WindowOpenedEvent.Raised.Subscribe(new AnonymousObserver<(object, global::Avalonia.Interactivity.RoutedEventArgs)>(t =>
        {
            if (t.Item1 is Window w) { lock (_lock) _open.Add(w); }
        }));
        Window.WindowClosedEvent.Raised.Subscribe(new AnonymousObserver<(object, global::Avalonia.Interactivity.RoutedEventArgs)>(t =>
        {
