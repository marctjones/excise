...
| `render perf` | `reference-performance` | excise's in-process `renderMs` relative to the oracles measured in the SAME run (absolute ms are load-dependent); `regressionGate` reported, never enforced. `--fixture <id>` / `--cohort <name>` narrow it to a ~2 s dev loop, and the ratio then uses only oracles present on both sides. The CLI self-reports `jit` vs `aot` and the gate REFUSES to compare across codegen modes (#1389) | mutool, pdftocairo, Ghostscript, PDFBox |
