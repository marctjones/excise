...
/// issue #639). These are the excise-internal / "own decrypt path" checks
...
/// Excise.Rendering.Tests/Differential/EncryptionWriterInteropTests.cs.
/// </summary>
public class PdfDocumentWriterEncryptionTests
{
    ...
    #region Round-trip via excise's own decrypt handler (supplementary only — see class remarks)
    ...
    // The excise-internal / "own decrypt path" checks for R=4, same caveat as
    ...
    public void Write_UserPasswordOnly_OwnerEntriesDoNotValidateAgainstTheEmptyPassword()
    {
        ...
        foreach (var algorithm in new[] { PdfEncryptionAlgorithm.Aes256, PdfEncryptionAlgorithm.Aes128 })
        {
            ...
            openEmpty.Should().Throw<Excise.Core.Parsing.PdfEncryptionNotSupportedException>(
