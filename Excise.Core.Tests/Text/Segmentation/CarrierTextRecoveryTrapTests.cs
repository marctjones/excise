...
public class CarrierTextRecoveryTrapTests
{
    ...
    public void CarrierScan_HasNoProductionCallerOutsideUnredact()
    {
        ...
        // Both callers ARE unredact: the CLI handler, and the recovery scan that
        // backs it (#1587). RecoveryScanner is reached only from `unredact` — no
        // open, render or redact path constructs one — so the rule this trap
        // exists to hold is intact. Adding a caller here means proving that
        // again, not widening the list.
        callers.Should().Equal(new[]
        {
            "Excise.Cli/Commands/UnredactCommandHandler.cs",
            "Excise.Core/Redaction/Recovery/RecoveryScanner.cs",
