...
public class RtlType0ExtractionTests
{
    ...
    public void LtrLabelThenRtlName_ExtractsLogicalOrder_AndSpanningPhraseRedacts()
    {
        ...
        using (var doc = PdfDocument.Open(Type0RtlFixtures.SingleLine(visual)))
            doc.RedactText(Str(khalid), RedactionOptions.Default).VerifiedRemovals.Should().BeGreaterThan(0, "per-word redaction");

        using (var doc = PdfDocument.Open(Type0RtlFixtures.SingleLine(visual)))
            doc.RedactText("Name " + Str(khalid), RedactionOptions.Default).VerifiedRemovals.Should().BeGreaterThan(0,
    ...
    public void Limitation_WholeLineUba_RtlBaseTrailingLtr_PerWordRedactsButSpanningPhraseDoesNot()
    {
        ...
        using (var doc = PdfDocument.Open(Type0RtlFixtures.SingleLine(visual)))
            doc.RedactText(word, RedactionOptions.Default).VerifiedRemovals.Should().BeGreaterThan(0,
        ...
        using (var doc = PdfDocument.Open(Type0RtlFixtures.SingleLine(visual)))
            doc.RedactText(word + " hello", RedactionOptions.Default).VerifiedRemovals.Should().Be(0,
