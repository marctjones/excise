...
/// <summary>
/// Finds and removes every embedded file a PDF can carry (#1572). What a
/// redaction does with the files it keeps is
/// <c>Excise.Core.Text.Segmentation.AttachmentCarrierScrubber</c>'s job.
/// </summary>
/// <remarks>
...
/// </remarks>
internal static class PdfAttachmentGraph
{
    ...
    internal static byte[]? DecodedBytes(PdfStream? stream)
    {
    ...
    internal static long? SizeOf(PdfDocument document, Found file)
    {
    ...
    internal static PdfDictionary? ResolveDict(PdfDocument document, PdfObject? value)
        => value == null ? null : document.Resolve(value) as PdfDictionary;

    internal static void RemoveKey(PdfDictionary dictionary, string key, List<Action>? undo)
    {
