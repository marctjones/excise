...
internal static class JpxDecoder
{
    ...
    private static JpxImage? TryDecodeOpenJpeg(byte[] jpxData, int reduceFactor, string outputFileName)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), "excise-openjpeg-" + Guid.NewGuid().ToString("N"));
        try
        {
    ...
    {
        var configured = Environment.GetEnvironmentVariable("EXCISE_OPENJPEG_DECOMPRESS");
