...
public static class PdfCommandRegistry
{
    ...
    [
        ...
        App(PdfCommandIds.Undo, "Undo", "Undo the last document edit.", "Ctrl+Z", requiresDocument: true),
        App(PdfCommandIds.Redo, "Redo", "Redo the last undone edit.", "Ctrl+Y", requiresDocument: true),
        ...
        App(PdfCommandIds.TypewriterGoToNextPendingEdit, "Go To Next Pending Typewriter Edit",
            "Move to the next unapplied typewriter edit.", requiresDocument: true),
        App(PdfCommandIds.JumpToSearchMatch, "Jump To Search Match",
            "Scroll to a search result.", requiresDocument: true),
        ...
        App(PdfCommandIds.Documentation, "Documentation", "Open excise documentation."),
        App(PdfCommandIds.About, "About excise", "Show excise version and license information."),
        ...
        Annotate(PdfCommandIds.ToggleHighlightMode, "Highlight Tool", "Toggle highlight-markup mode; selecting text becomes a Highlight the instant the selection finishes."),
        Annotate(PdfCommandIds.ToggleUnderlineMode, "Underline Tool", "Toggle underline-markup mode; selecting text becomes an Underline the instant the selection finishes."),
        Annotate(PdfCommandIds.ToggleStrikeOutMode, "Strikeout Tool", "Toggle strikeout-markup mode; selecting text becomes a Strikeout the instant the selection finishes."),
        Annotate(PdfCommandIds.ToggleSquigglyMode, "Squiggly Tool", "Toggle squiggly-markup mode; selecting text becomes a Squiggly-underline the instant the selection finishes."),
        ...
        Annotate(PdfCommandIds.ToggleSquareMode, "Draw Square", "Toggle square drawing; dragging on the page becomes a Square annotation."),
        Annotate(PdfCommandIds.ToggleCircleMode, "Draw Circle", "Toggle circle drawing; dragging on the page becomes a Circle (ellipse) annotation."),
        Annotate(PdfCommandIds.ToggleFreeTextMode, "Draw Text Box", "Toggle text-box drawing; dragging on the page, then a text prompt, becomes a FreeText annotation."),
        Annotate(PdfCommandIds.ToggleStampMode, "Draw Stamp", "Toggle stamp drawing; dragging on the page places the selected preset stamp."),
        Annotate(PdfCommandIds.ToggleImageStampMode, "Draw Image Stamp", "Toggle image-stamp drawing; dragging on the page, then an image-file prompt, places the chosen image."),
        ...
        Document(PdfCommandIds.Attachments, "Attachments", "Show the Attachments pane: files embedded in the document, which can be saved to disk or stripped."),
        Document(PdfCommandIds.SaveAttachment, "Save Attachment", "Save the selected embedded file to a location you choose."),
        Document(PdfCommandIds.SaveAllAttachments, "Save All Attachments", "Save every embedded file into a folder you choose."),
        Document(PdfCommandIds.RemoveAllAttachments, "Remove All Attachments", "Strip the document's embedded files; save afterwards to keep the change.", isDestructive: true),
        Document(PdfCommandIds.BatesNumbering, "Bates Numbering", "Stamp a sequential Bates number on every page of the open document."),
        Document(PdfCommandIds.ReduceFileSize, "Reduce File Size", "Write a smaller copy of the document: recompress, deduplicate, and optionally downsample images. The original is never overwritten.",
            cliCommand: "optimize",
            parameters: [Param("input", "Source PDF file path.", "file", true), Param("output", "Output PDF path; must differ from the input.", "file", true), Param("preset", "lossless, high, standard, or screen.", "string")],
            resultFields: ["inputSizeBytes", "outputSizeBytes", "sizeRatio"]),
        ...
        View(PdfCommandIds.ToggleAttachments, "Toggle Attachments Pane", "Show or hide the pane listing the document's embedded files.", null),
        ...
        View(PdfCommandIds.ToggleCommentAnnotations, "Show Comments",
            "Show or hide comment annotations — notes, highlights, shapes, ink and stamps.", null),
        View(PdfCommandIds.ToggleFieldAndLinkAnnotations, "Show Form Fields and Links",
            "Show or hide form fields and links. Their values are page content, so this is separate from comments.", null),
        View(PdfCommandIds.ToggleAnnotationAuditMode, "Reveal Hidden Annotations (Audit)",
            "Draw annotations the file marks Hidden or NoView. Off by default: it shows what no conforming viewer shows, and never affects an export.", null),
        ...
        Cli(PdfCommandIds.ExtractText, "Extract Text", "Extract text from a PDF.", "text",
            parameters: [Param("file", "PDF file.", "file", true), Param("page", "Optional page number.", "integer"),
                Param("ignorePermissions", "Override the document's /P copy/extract restriction (#642).", "boolean"),
                Param("forAccessibility", "Invoke the /P bit 10 extract-for-accessibility carve-out (#642).", "boolean")],
            resultFields: ["text"]),
        Cli(PdfCommandIds.ShowLetters, "Show Letters", "Show extracted letters with positions.", "letters",
            parameters: [Param("file", "PDF file.", "file", true), Param("page", "Page number.", "integer"),
                Param("ignorePermissions", "Override the document's /P copy/extract restriction (#642).", "boolean"),
                Param("forAccessibility", "Invoke the /P bit 10 extract-for-accessibility carve-out (#642).", "boolean")],
            resultFields: ["letters"]),
    ...
    public static PdfCommandMetadata Get(string id)
    {
        ...
        return ById.TryGetValue(id, out var command)
            ? command
            : throw new KeyNotFoundException($"Unknown excise command id '{id}'.");
    }
