 ...
 for section in registry['sections']:
  for cap in load(REG/section['path'])['capabilities']:
   ...
   for mode,state in cap['modes'].items():
    ...
    # Grade (#1346): a passing EXPLICIT contract earns credit; a test-
    # candidate match (keyword/facet discovery, testCandidates below) never
    # does -- that is the exact "discovery is not credit" line #1344 existed
    # to defend once its refs pointed at real tests. 'verified' additionally
    # requires one of those passing checks to carry kind='differential', i.e.
    # a reference tool that is not excise (registry.json verificationKinds);
    # 'unit'/'atomic-fixture'/'corpus'/'security'/'benchmark' alone stay
    # 'implemented' -- excise checking its own output is not the same claim.
    grade='verified' if any(c['kind']=='differential' and c['recordedOutcomes']==['Passed'] for c in explicit) else 'implemented' if contract_status=='passing' else 'unknown'
    rows.append({'capability':cap['id'],'section':section['id'],'mode':mode,'currentState':state,'testCandidates':found[:160],'explicitContracts':explicit,'explicitContractStatus':contract_status,'grade':grade,'benchmarkScenarios':[b['id'] for b in benchmarks if cap['id'] in b['capabilities']]})
 summary={'targetModes':len(rows),'modesWithTestCandidates':sum(bool(r['testCandidates']) for r in rows),'modesWithExplicitContracts':sum(bool(r['explicitContracts']) for r in rows),'modesWithPassingExplicitContracts':sum(r['explicitContractStatus']=='passing' for r in rows),'modesWithRecordedPassingCandidate':sum(any(x['recordedOutcomes']==['Passed'] for x in r['testCandidates']) for r in rows),'gradeImplemented':sum(r['grade']=='implemented' for r in rows),'gradeVerified':sum(r['grade']=='verified' for r in rows),'gradeUnknown':sum(r['grade']=='unknown' for r in rows),'testMethods':len(all_tests),'testMethodsWithCapabilityCandidate':sum(bool(r['parentCapabilityModes']) for r in all_tests.values()),'benchmarkScenarios':len(benchmarks),'benchmarkScenariosWithHarness':sum(b['status']=='existing-harness' for b in benchmarks)}
 OUT.write_text(json.dumps({'schemaVersion':1,'generatedBy':'scripts/build-pdf-evidence-attribution.py','policy':'Candidate attribution measures discoverable test/benchmark relevance. Only explicit reviewed contracts affect strict support; test outcomes are recorded-run evidence only. grade (#1346): implemented requires a passing explicit contract, verified additionally requires one of kind differential (an independent oracle) -- a testCandidate match never grades on its own.','modes':rows,'summary':summary},indent=2)+'\n'); print(f'wrote {OUT} with {summary["modesWithTestCandidates"]}/{summary["targetModes"]} modes having test candidates, {summary["gradeImplemented"]} implemented, {summary["gradeVerified"]} verified')
