...
def implementation_evidence_progress(row, mode, collected, attribution):
    """Fair progress credit, capped below strict until a reviewer promotes it."""
    if row["modes"][mode] == "implemented":
        return 100
    item = attribution.get((row["id"], mode), {})
    checks = item.get("explicitContracts", [])
    score = 10 if item.get("testCandidates") or collected.get(row["id"], {}).get("candidateReferenceCount") else 0
    if checks: score += 20
    if row["modes"][mode] == "partial": score += 20
    if item.get("explicitContractStatus") == "passing": score += 20
    if any(check["kind"] == "atomic-fixture" for check in checks): score += 15
    if any(check["kind"] in {"differential", "corpus"} for check in checks): score += 15
    if item.get("benchmarkScenarios"): score += 5
    return min(score, 95)
...
def summary(rows, collected, attribution=None):
    ...
    return {
        ...
        "implementationEvidenceProgressPercent": percent(sum(implementation_progress), len(implementation_progress) * 100),
