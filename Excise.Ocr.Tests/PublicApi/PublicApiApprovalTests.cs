...
/// <summary>
/// Public-API snapshot for <c>Excise.Ocr</c> (#910). Unlike the packable-library
...
{
    [Fact]
    public void ExciseOcr_PublicApi_MatchesApprovedBaseline()
    {
        var api = typeof(Excise.Ocr.PdfOcrService).Assembly
            .GeneratePublicApi(new ApiGeneratorOptions { IncludeAssemblyAttributes = false })
            .Replace("\r\n", "\n")
            .TrimEnd() + "\n";
        var file = Path.Combine(ApprovedDir(), "Excise.Ocr.approved.txt");
        ...
        api.Should().Be(approved,
            "the Excise.Ocr public API must not change without an intentional review. " +
            "If deliberate, re-run with APPROVE_PUBLIC_API=1 and commit the updated Excise.Ocr.approved.txt.");
    }
