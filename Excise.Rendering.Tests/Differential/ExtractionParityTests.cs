...
/// case below), extracts with excise and with mutool and records:
///   - coverage ratio (excise Unicode letter/digit count ÷ mutool's) — catches
...
public sealed class ExtractionParityTests
{
    ...
    public void GenerateExtractionParityReport()
    {
        ...
        var root = LocateRepoRoot();
        Assert.SkipWhen(root == null, "could not locate repo root (excise.sln not found above AppContext.BaseDirectory)");
        ...
        // read clean for a reason that has nothing to do with excise being fixed.
        var corpusDirs = new[]
        {
        ...
        long exciseTotal = pages.Sum(p => (long)p.ExciseChars);
        long mutoolTotal = pages.Sum(p => (long)p.MutoolChars);
        double aggregateCoverage = mutoolTotal == 0 ? 1.0 : (double)exciseTotal / mutoolTotal;
        ...
        _output.WriteLine($"Aggregate coverage: excise extracts {aggregateCoverage:P1} of mutool's Unicode letter/digit count");
    ...
    /// case — excise silently extracting near-nothing from a file mutool can
    /// still read a meaningful amount of text from, which is exactly the
    /// "redact what excise can't see" failure #637/#645 are about, just
    ...
    [Fact]
    public void AdversarialCorpus_ExciseDoesNotCatastrophicallyUnderExtractRelativeToMutool()
    {
        Assert.SkipUnless(MutoolReferenceRenderer.IsAvailable,
        ...
        foreach (var pdfPath in pdfPaths)
        {
            ...
            int exciseChars = CountLetterOrDigit(exciseText);
            double coverage = (double)exciseChars / mutoolChars;
        ...
        newDisagreements.Should().BeEmpty(
            "excise extracting <50% of what mutool reads from a file both tools opened is a redaction-relevant " +
    ...
    private static PageEntry ScanPage(string relPath, string pdfPath, PdfDocument doc, int pageNumber)
    {
        ...
        try
        {
            exciseText = doc.GetPage(pageNumber).Text ?? "";
        }
        catch (Exception ex)
        {
        ...
        entry.ExciseChars = CountLetterOrDigit(exciseText);
        entry.MutoolChars = CountLetterOrDigit(mutoolText);
        entry.CoverageRatio = TextSimilarity.CoverageRatio(exciseText, mutoolText);
        entry.Similarity = TextSimilarity.BigramJaccard(exciseText, mutoolText);
    ...
    private static string DescribeEncoding(Excise.Core.Primitives.PdfDictionary font)
    {
        var encObj = font.GetOptional("Encoding");
        return encObj switch
        {
            Excise.Core.Primitives.PdfName n => n.Value,
            Excise.Core.Primitives.PdfDictionary => "(dictionary)",
            null => "(none)",
    ...
    public sealed class PageEntry
    {
        ...
        public int ExciseChars { get; set; }
