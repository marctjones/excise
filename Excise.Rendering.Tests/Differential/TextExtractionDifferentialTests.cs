...
/// corpus PDF with both excise and mutool, compare on a normalized form.
...
public sealed class TextExtractionDifferentialTests
{
    ...
    public void TextMatchesMutool(string relativePath)
    {
        ...
        try
        {
            ...
            exciseText = doc.GetPage(1).Text;
        }
        catch (Exception ex)
        {
            Assert.SkipWhen(true,
                $"excise could not extract from {relativePath}: {ex.GetType().Name}: {ex.Message}");
        }
        ...
        var ourNormalized = TextSimilarity.Normalize(exciseText ?? string.Empty);
        var sim = TextSimilarity.BigramJaccard(refNormalized, ourNormalized);

        _output.WriteLine($"  {relativePath}");
        _output.WriteLine($"  excise:   {Truncate(ourNormalized, 96)}");
        ...
        sim.Should().BeGreaterThanOrEqualTo(MinSimilarity,
            $"{relativePath}: excise text-extraction differs from mutool. " +
