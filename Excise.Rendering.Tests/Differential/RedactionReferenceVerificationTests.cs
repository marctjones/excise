...
/// <summary>
/// Verifies redacted output with tools that are not excise (issue #606).
///
/// Every redaction test we own asks excise whether excise removed the text. That is
...
///
/// Both leaks were invisible to excise's own extractor and glaringly visible to
...
///      any carrier excise's extractor cannot see — structure tree, ToUnicode CMap
...
public class RedactionReferenceVerificationTests : IDisposable
{
    ...
    public void RedactedText_IsNotRecoverableByAnIndependentExtractor()
    {
        ...
        extracted!.Should().NotContain(Secret,
            "mutool reads carriers excise's own extractor does not — the structure tree, the " +
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"excise-redact-{Guid.NewGuid():N}.pdf");
        pdf.Save(path);
        _temp.Add(path);
        return path;
    }
