...
/// be its own oracle for the property it exists to guarantee": excise
/// confirming excise's own output decrypts with excise's own decrypt handler
/// (see <c>Excise.Core.Tests/Writing/PdfDocumentWriterEncryptionTests.cs</c>)
...
///
/// These tests instead ask three tools excise does not control — qpdf,
...
public class EncryptionWriterInteropTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"excise-enc-interop-{Guid.NewGuid():N}");
    ...
    {
        // Same discriminating property as the excise-internal test
        // (Write_Aes128_MultipleObjectsWithIdenticalPlaintext_ProduceDifferentCiphertext
        // in Excise.Core.Tests), checked here against qpdf's independent
        // decrypt instead of excise's own: two objects sharing identical
        // plaintext must still both decrypt correctly via a reader excise
        // does not control.
        Assert.SkipUnless(QpdfReferenceTool.IsAvailable, "qpdf not on PATH");
    ...
    [Fact]
    public void QpdfWrittenR4File_ExciseIndependentlyDecryptsIt()
    {
        // The reverse-direction oracle (see QpdfReferenceTool.EncryptR4's
        // remarks): a file whose /O, /U, and per-object AES-128 ciphertext
        // were produced entirely by qpdf, with zero excise code involved in
        // the encrypt side. A successful read here is the strongest
        // evidence excise's shared KDF helpers (DeriveFileKey, ComputeObjectKey,
        // the R>=3 RC4 chain) are actually spec-correct, not just
        // self-consistent between excise's own encrypt and decrypt halves.
        Assert.SkipUnless(QpdfReferenceTool.IsAvailable, "qpdf not on PATH");
