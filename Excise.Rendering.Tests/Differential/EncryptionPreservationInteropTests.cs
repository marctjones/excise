...
/// excise. And because the saved bytes are ciphertext, a naive byte-scan of
...
public class EncryptionPreservationInteropTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"excise-enc-preserve-{Guid.NewGuid():N}");
    ...
    public void Redact_R6Source_OutputSameR6AndPermissions_SecretUnrecoverableByQpdfAndMutool()
    {
        ...
        // mutool's extraction, neither of which is excise.
    ...
    public void Redact_Rc4Fixture_UpgradesToR6_PermissionsSurvive_SecretUnrecoverable()
    {
        ...
        // (qpdf-verified), page text contains "Issue 15893". excise's writer
        ...
        {
            doc.RedactText(fixtureSecret, RedactionOptions.Default with { DrawBox = false }).VerifiedRemovals.Should().BeGreaterThan(0,
    ...
    public void Preserve_EncryptMetadataFalse_SurvivesRoundTrip()
    {
        ...
        // raw byte scan of the OUTPUT (not a excise reparse) can verify it.
    ...
    /// <summary>
    /// A excise-authored single-page document whose text contains both the
    ...
    private string RedactAndSavePreserved(string sourcePath, string password)
    {
        ...
        doc.RedactText(Secret, RedactionOptions.Default with { DrawBox = false }).VerifiedRemovals.Should().BeGreaterThan(0,
