...
///
///   1. Extract text via excise (page.Text).
...
///
/// This is the security-critical test: the excise value proposition is
...
///     into a raster image XObject. That's <c>excise audit --deep</c>.
    ...
    public void RedactedWordIsGoneAfterSaveAndReopen(string relativePath)
    {
        ...
        catch (Exception ex)
        {
            Assert.SkipWhen(true,
                $"excise could not open {relativePath}: {ex.GetType().Name}: {ex.Message}");
            return;
        }
        ...
        {
            throw new Xunit.Sdk.XunitException(
                $"{relativePath}: excise couldn't reopen its own redacted output: " +
