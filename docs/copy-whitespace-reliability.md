...
| **Headers / footers** | **No longer defeat column detection (#774/#824).** | A continuous full-width running header/footer/title/page-number line used to fill the gutter in the X-sweep and force the whole page to woven row-major order. It is now excluded from gutter detection and emitted in place, band-separating the columns (proven by `TwoColumnHeaderReadingOrderTests`). Still copies inline within its band. |
| **Multi-column** (via #774/#824) | **Handles two-column-with-header** — a full-width header/footer no longer scrambles a clean two-column page (band-segmented, column-by-column). | Bounded to a *wide* gutter and the two-column case: narrow gutters, 3+ columns, baseline-aligned tables, and pages whose extraction itself interleaves glyphs at the same Y (scotus — an extraction-geometry issue, not reading order) are out of scope and degrade to geometric order. Conservatism preserved: no qualifying gutter → `SortSimple`, single-column byte-identical. |
