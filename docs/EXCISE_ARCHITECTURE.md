...
```
Excise.Core           - PDF parsing, document model, modification, writing
Excise.Rendering      - Page rendering to images (SkiaSharp)
Excise.Core/Operations - High-level operations that are implemented in this repo
Excise.Cli            - Command-line tools
Excise.App           - Desktop application (Avalonia)
```

---

## Excise.Core Architecture

### Layer 1: Primitives (`Excise.Core.Primitives`)
...
### Layer 2: Parsing (`Excise.Core.Parsing`)
...
### Layer 3: Document Model (`Excise.Core.Document`)
...
### Layer 4: Content Stream (`Excise.Core.Content`)
...
### Layer 5: Text Extraction (`Excise.Core.Text`)
...
### Layer 6: Fonts (`Excise.Core.Fonts`)
...
### Layer 7: Graphics (`Excise.Core.Graphics`)
...
### Layer 8: Writing (`Excise.Core.Writing`)
...
High-level operations are currently implemented inside `Excise.Core`, not as a
separate `Excise.Operations` assembly:
...
PDF encryption reading is implemented in `Excise.Core/Security`. Encryption
...
excise-owned libraries in this repository. Legacy PdfPig/PDFsharp/PDFtoImage
