...
| Thumbnail sidebar | `ThumbnailSidebarSession` owns demand, prefetch/prewarm cancellation, and displayed Avalonia bitmaps; `ThumbnailCacheService` owns serialized raster requests and the persistent WebP cache. The whole-document prewarm starts only after a quiet period (`PrewarmIdleDelay`, pushed in from the `IdleTrimSeconds` preference — the app's one idle delay, 30 s by default) that any document activity restarts, and warms the disk cache through `WarmAsync`, which produces no bitmap (#1565). | Session reset/document replacement cancels background demand and disposes displayed bitmaps. The persistent cache is content/version keyed and trims least-recently-used document directories to 500 MiB; a missing or trimmed entry is regenerated. |
