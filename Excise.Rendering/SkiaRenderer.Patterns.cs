...
internal partial class RenderContext
{
    ...
    private bool RenderFillPattern(SKPath path)
    {
        ...
        if (shadingObj == null)
            return false;
        var shading = _page.Document.Resolve(shadingObj) as Excise.Core.Primitives.PdfDictionary;
        if (shading == null)
            return false;
    ...
    private bool RenderTilingPattern(SKPath clipPath, Excise.Core.Primitives.PdfDictionary pattern)
    {
        if (pattern is not Excise.Core.Primitives.PdfStream stream)
            return false;
        ...
        var bboxArray = pattern.GetOptional("BBox") as Excise.Core.Primitives.PdfArray;
    ...
    private bool RenderShadingPattern(
        SKPath clipPath,
        Excise.Core.Primitives.PdfDictionary pattern,
    ...
    private List<MeshTriangle> DecodeType4MeshTriangles(Excise.Core.Primitives.PdfStream stream)
    {
    ...
    private List<MeshTriangle> DecodeType5MeshTriangles(Excise.Core.Primitives.PdfStream stream)
    {
        var decode = stream.GetOptional("Decode") as Excise.Core.Primitives.PdfArray;
        var xMin = decode?.Count >= 2 ? decode.GetNumber(0) : 0;
        var xMax = decode?.Count >= 2 ? decode.GetNumber(1) : 1;
    ...
    private List<MeshPatch> DecodeType6MeshPatches(Excise.Core.Primitives.PdfStream stream, bool tensorPatch)
    {
        var decode = stream.GetOptional("Decode") as Excise.Core.Primitives.PdfArray;
        var xMin = decode?.Count >= 2 ? decode.GetNumber(0) : 0;
        var xMax = decode?.Count >= 2 ? decode.GetNumber(1) : 1;
    ...
    private static int GetMeshComponentCount(
        ...
        Excise.Core.Primitives.PdfObject? function)
    {
        if (function != null)
            return 1;

        if (stream.GetOptional("Decode") is Excise.Core.Primitives.PdfArray decode && decode.Count > 4)
    ...
    private static (bool Start, bool End) GetShadingExtend(Excise.Core.Primitives.PdfDictionary shading)
    {
        if (shading.GetOptional("Extend") is not Excise.Core.Primitives.PdfArray extend)
            return (false, false);

        return (
            extend.Count > 0 && extend[0] is Excise.Core.Primitives.PdfBoolean start && start.Value,
            extend.Count > 1 && extend[1] is Excise.Core.Primitives.PdfBoolean end && end.Value);
    }

    private void ApplyShadingBoundingBoxClip(Excise.Core.Primitives.PdfDictionary shading)
    {
    ...
    private SKRect? GetShadingBoundingBox(Excise.Core.Primitives.PdfDictionary shading)
    {
        ...
        Excise.Core.Primitives.PdfDictionary shading)
    {
        ...
        var funcObj = funcRef != null ? _page.Document.Resolve(funcRef) : null;
        var domain = GetNumberArray(shading.GetOptional("Domain") as Excise.Core.Primitives.PdfArray)
                     ?? GetNumberArray((funcObj as Excise.Core.Primitives.PdfDictionary)?.GetOptional("Domain") as Excise.Core.Primitives.PdfArray)
                     ?? new[] { 0.0, 1.0 };
    ...
    private PdfColorSpace ResolveShadingColorSpace(Excise.Core.Primitives.PdfDictionary shading)
    {
    ...
    private PdfObject? ResolveColorSpaceObject(string name)
    {
        foreach (var resources in _resourcesStack)
        {
            ...
            if (_page.Document.Resolve(colorSpacesObj) is not Excise.Core.Primitives.PdfDictionary colorSpaces)
                continue;
    ...
    /// </summary>
    private Excise.Core.Primitives.PdfDictionary? ResolveFontFromActiveResources(string fontName)
    {
        foreach (var resources in _resourcesStack)
        {
            if (resources == null) continue;
            ...
            if (_page.Document.Resolve(fontsObj) is not Excise.Core.Primitives.PdfDictionary fonts)
                continue;
            var fontObj = fonts.GetOptional(fontName);
            if (fontObj == null) continue;
            return _page.Document.Resolve(fontObj) as Excise.Core.Primitives.PdfDictionary;
        }
        return null;
    }
    ...
    /// </summary>
    private Excise.Core.Primitives.PdfObject? ResolveXObjectFromActiveResources(string name)
    {
        foreach (var resources in _resourcesStack)
        {
            if (resources == null) continue;
            ...
            if (_page.Document.Resolve(xobjsObj) is not Excise.Core.Primitives.PdfDictionary xobjs)
                continue;
    ...
    private Excise.Core.Primitives.PdfDictionary? ResolveExtGStateFromActiveResources(string name)
    {
        foreach (var resources in _resourcesStack)
        {
            if (resources == null) continue;
            ...
            if (_page.Document.Resolve(extGStatesObj) is not Excise.Core.Primitives.PdfDictionary extGStates)
                continue;
            var extGState = extGStates.GetOptional(name);
            if (extGState == null) continue;
            return _page.Document.Resolve(extGState) as Excise.Core.Primitives.PdfDictionary;
        }

        return null;
    }

    private Excise.Core.Primitives.PdfObject? ResolvePropertyFromActiveResources(string name)
    {
        foreach (var resources in _resourcesStack)
        {
            if (resources == null) continue;
            ...
            if (_page.Document.Resolve(propertiesObj) is not Excise.Core.Primitives.PdfDictionary properties)
                continue;
    ...
    private Excise.Core.Primitives.PdfDictionary? ResolvePatternFromActiveResources(string name)
    {
        foreach (var resources in _resourcesStack)
        {
            if (resources == null) continue;
            ...
            if (_page.Document.Resolve(patternsObj) is not Excise.Core.Primitives.PdfDictionary patterns)
                continue;
            var pattern = patterns.GetOptional(name);
            if (pattern == null) continue;
            return _page.Document.Resolve(pattern) as Excise.Core.Primitives.PdfDictionary;
        }

        return null;
    }

    private Excise.Core.Primitives.PdfDictionary? ResolveShadingFromActiveResources(string name)
    {
        foreach (var resources in _resourcesStack)
        {
            if (resources == null) continue;
            ...
            if (_page.Document.Resolve(shadingsObj) is not Excise.Core.Primitives.PdfDictionary shadings)
                continue;
            var shadingObj = shadings.GetOptional(name);
            if (shadingObj == null) continue;
            return _page.Document.Resolve(shadingObj) as Excise.Core.Primitives.PdfDictionary;
        }
